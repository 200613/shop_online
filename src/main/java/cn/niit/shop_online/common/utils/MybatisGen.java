...
public class MybatisGen {
    ...
    public static void main(String[] args) {
        String path = System.getProperty("user.dir").concat(File.separator).concat("src").concat(File.separator).concat("main")
                .concat(File.separator).concat("java").concat(File.separator).concat("cn").concat(File.separator).concat("niit").concat(File.separator).concat("shop_online").concat(File.separator);
        ...
        FastAutoGenerator.create("jdbc:mysql://localhost:3306/shop_online", "root", "123456")
                .globalConfig(builder -> {
                    builder.author("200613").enableSwagger(); // 设置作者// 开启 swagger 模式

                })
                .packageConfig(builder -> {
                    builder.parent("cn.niit") // 设置父包名
